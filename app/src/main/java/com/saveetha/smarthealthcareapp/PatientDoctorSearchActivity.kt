...
import com.saveetha.smarthealthcareapp.adapter.ClinicAdapter
...
class PatientDoctorSearchActivity : AppCompatActivity() {
    ...
    private lateinit var adapter: ClinicAdapter
     private lateinit var langEnglish: TextView
     private lateinit var langTelugu: TextView
     private lateinit var langHindi: TextView
     private lateinit var langTamil: TextView
     private lateinit var langBengali: TextView
     private lateinit var langMarathi: TextView
     private lateinit var langUrdu: TextView
     private lateinit var langGujarati: TextView
     private lateinit var langKannada: TextView
     private lateinit var langMalayalam: TextView
     private lateinit var langOdia: TextView
     private lateinit var langPunjabi: TextView

     private lateinit var languageViews: List<TextView>
     private var selectedLanguage: String? = null

    private val sampleStates = listOf("Select State", "Andhra Pradesh")
    private val sampleDistricts = mapOf(
        "Andhra Pradesh" to listOf(
            "Select District", "Alluri Sitharama Raju", "Anakapalli", "Ananthapuramu", "Annamayya", "Bapatla", "Chittoor",
            ...
            "Sri Sathya Sai", "Tirupati", "Visakhapatnam", "Vizianagaram", "West Godavari", "Y.S.R. Kadapa"
        )
    )


    private val districtToDivisionsMap = mapOf(
   ...
   private val divisionToMandalsMap = mapOf(
...
//            "Seethampeta" to listOf("Select GP", "Seethampeta", "Pachagonda", "Chinthapalli"),
   ...
   private val mandalToGPsMap = mapOf(
...
//...............................
     ...
     override fun onCreate(savedInstanceState: Bundle?) {
          ...
          setContentView(R.layout.activity_patient_doctor_search)
          ...
          langUrdu = findViewById(R.id.langUrdu)
          langGujarati = findViewById(R.id.langGujarati)
          langKannada = findViewById(R.id.langKannada)
          langMalayalam = findViewById(R.id.langMalayalam)
          langOdia = findViewById(R.id.langOdia)
          langPunjabi = findViewById(R.id.langPunjabi)

          languageViews = listOf(langEnglish, langTelugu, langHindi, langTamil, langBengali, langMarathi, langUrdu, langGujarati, langKannada, langMalayalam, langOdia, langPunjabi)
          ...
          searchButton = findViewById(R.id.btnSearchDoctors)
          ...
          setupSpinners()
          setupSearch()
          setupLanguageSelection() // Call this after initializing views
     }

     private fun setupLanguageSelection() {
          // Set up chip clicks
          for (lang in languageViews) {
          ...
          setLanguageSelection(langEnglish)
     }

     private fun setLanguageSelection(selectedLang: TextView) {
          for (lang in languageViews) {
               // Mark selected/unselected
               lang.isSelected = (lang == selectedLang)

               // Create drawable with background color + border
               val drawable = GradientDrawable()
               drawable.cornerRadius = 24f
               if (lang == selectedLang) {
                    drawable.setColor(Color.parseColor("#3F51B5"))  // Blue fill
                    drawable.setStroke(2, Color.parseColor("#3F51B5")) // Blue border
                    lang.setTextColor(Color.WHITE) // White text
               } else {
                    drawable.setColor(Color.WHITE) // White fill
                    drawable.setStroke(2, Color.parseColor("#9E9E9E")) // Gray border
                    lang.setTextColor(Color.BLACK) // Black text
               }

               lang.background = drawable
          }

          selectedLanguage = selectedLang.text.toString()
     }
     ...
     private fun setupSpinners() {
        ...
        spinnerState.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                val selectedState = sampleStates[position]
                val districts = sampleDistricts[selectedState] ?: listOf("Select District")

                val districtAdapter = ArrayAdapter(
                    this@PatientDoctorSearchActivity,
        ...
        spinnerDistrict.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                ...
                spinnerDivision.adapter = ArrayAdapter(this@PatientDoctorSearchActivity, android.R.layout.simple_spinner_dropdown_item, divisions)
            }
        ...
        spinnerDivision.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                ...
                spinnerMandal.adapter = ArrayAdapter(this@PatientDoctorSearchActivity, android.R.layout.simple_spinner_dropdown_item, mandals)
            }
        ...
        spinnerMandal.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                ...
                spinnerGP.adapter = ArrayAdapter(this@PatientDoctorSearchActivity, android.R.layout.simple_spinner_dropdown_item, gps)
            }
    ...
    private fun setupSearch() {
        searchButton.setOnClickListener {
            ...
            val block = spinnerDivision.selectedItem.toString()
            val mandal = spinnerMandal.selectedItem.toString()
            val gp = spinnerGP.selectedItem.toString()
             val language = selectedLanguage ?: "Telugu"

            if (state == "Select State" || district == "Select District" || block == "Select Division" || mandal == "Select Mandal" || gp == "Select GP") {
                Toast.makeText(this, "Please select all fields", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            ApiClient.instance.searchClinics(state, district, block, mandal, gp, language)
                .enqueue(object : Callback<List<Clinic>> {
                    override fun onResponse(call: Call<List<Clinic>>, response: Response<List<Clinic>>) {
                        ...
                        if (clinics != null && clinics.isNotEmpty()) {
                            adapter = ClinicAdapter(clinics)
                            recyclerView.adapter = adapter
                        } else {
                            Toast.makeText(this@PatientDoctorSearchActivity, "No clinics found", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onFailure(call: Call<List<Clinic>>, t: Throwable) {
                        Toast.makeText(this@PatientDoctorSearchActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                })
        }
    }
